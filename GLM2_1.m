...
analysis_dir =  [project_dir 'analysis\' 'mod\' 'noBC\' 'mod_x_perf'];

D=spm_eeg_load(fullfile(project_dir, 'processed_data', ['subj' subjects{1}],['sqrt_mtf_fraeMMMfxdisc' subjects{1} '.mat'])); %just load the chanlabels once
Elecs = D.chanlabels;
...
for k=1:length(subjects)
    ...
    for el = 1:length(Elecs);
            cont=0;
            for jj=1:numel(dontinclude)
                if strcmp(Elecs{el},dontinclude(jj))
                    cont=1;
                end
            end
            if cont==1; continue; end
...
%                 n2s = [];
                ...
                for m=1:3 %modalities: VisAud = 1; VisTac = 2; AudTac = 3      
                    for f=0:1
                        zelle=zelle+1;
                        modality=str2num(actdirs(:,end-4:end-3)); %modality !!!! CHECK!!!! -> VisAud = 1; VisTac = 2; AudTac = 3
...
%             matlabbatch{1}.spm.stats.factorial_design.cov(2).iCC = 1;
            ...
            cname       = 'all-mods-corr-incorr';
            c           =[ -1/6 1/6 -1/6 1/6 -1/6 1/6 ];
            SPM.xCon(4) = spm_FcUtil('Set',cname,'T','c',c',SPM.xX.xKXs);
            
            cname       = 'all-mods-incorr-corr';
            c           =[ 1/6 -1/6 1/6 -1/6 1/6 -1/6 ];
