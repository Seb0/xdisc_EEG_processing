...
D=spm_eeg_load(fullfile(project_dir, 'processed_data', 'subj02',['rtf_fraeMMMfxdisc' subjects{1} '.mat'])); %just load the chanlabels once
Elecs = D.chanlabels;
...
%             for k=1:length(subjects)
                ...
                for m=1:3 %modalities: VisAud = 1; VisTac = 2; AudTac = 3                 
                        ...
                        thisdirs=actdirs(modality==m,:); % ; % only correct trials? insert: & fps==1
                        selimg=[]; 
